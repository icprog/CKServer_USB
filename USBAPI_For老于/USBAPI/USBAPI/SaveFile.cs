...
namespace USBSpeedTest
{
    class SaveFile
    {
       ...
       //    FileCreateDat(Program.GetStartupPath() + @"源码\", out file0);
            ...
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF08\", out file_SC2);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF01\", out file_SC3);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF02\", out file_SC4);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF03\", out file_SC5);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF04\", out file_SC6);

            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF08\1D00(CAN数据)\", out file_SC7);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF08\1D01\", out file_SC8);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF08\1D02\", out file_SC9);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF08\1D03\", out file_SC10);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF08\1D04(AD1)\", out file_SC11);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF08\1D05(AD2)\", out file_SC12);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF08\1D06(AD3)\", out file_SC13);
            FileCreateDat(Program.GetStartupPath() + @"存储数据\FF08\1D07(AD4)\", out file_SC14);
        ...
        public void FileSaveStart()
        {
           ...
           // new Thread(() => { WriteToFileSC(0, file0, ref DataQueue_0, ref Lock); }).Start();
            ...
            new Thread(() => { WriteToFileSC(8, file_SC8, ref DataQueue_SC8, ref Lock_8); }).Start();
            new Thread(() => { WriteToFileSC(9, file_SC9, ref DataQueue_SC9, ref Lock_9); }).Start();
            new Thread(() => { WriteToFileSC(10, file_SC10, ref DataQueue_SC10, ref Lock_10); }).Start();
            new Thread(() => { WriteToFileSC(11, file_SC11, ref DataQueue_SC11, ref Lock_11); }).Start();
